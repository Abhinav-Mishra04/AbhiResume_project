    ...
    if(Array.isArray(el.o_data) && el.o_data.length > 0)
      {
        for(let i=0; i<el.o_data.length ; i++)
          {
              total = total + Number(el.o_data[i].quantity) * (el.o_data[i].pro_data.price - (el.o_data[i].pro_data.discount / 100)*el.o_data[i].pro_data.price)
          }
      }
         console.log(total)
      return total
...
<div class="card" style={{width : "90%"}}>
  ...
  <div class="card-body">
    <h5 class="card-title">Order Date : {moment(el.data).format('MMMM Do YYYY, h:mm:ss a')}</h5>
    <h5 class="card-title">Order Number : {el.ord_number}</h5>
    <h5 class="card-title">Payment Mode : {el.payment_mode}</h5>
