...
function Cart(){
  const navigate = useNavigate()
    ...
    function getCartCount(){
     ...
     console.log(res.data.data,"my cart item")
...
toast.error(err.response.data.message)
     ...
     const decre =(item)=>{
            axios.post(Base_URL + '/update-cart-quantity' , {c_id : item._id, case : "DECRE"}).then((res)=>{
              toast.success(res.data.message)
              window.location.reload()
     }).catch((err)=>{
      toast.error(err.response.data.message)
...
{data.map((el,i)=>(
  ...
  <div class="card-body">
    ...
    <input style={{textAlign:"center" ,width: "20px" , height:"35px" ,marginLeft : 1 , marginRight: 1}} disabled={true} value={el.quantity} defaultValue={1}/> 
    ...
    {/* <button disable={el.disable == true ? true : false} onClick={()=>{addToCart(el)}} class="btn btn-success">{el.disable == true ? "Already Aded":'Add To Cart'}</button> */}
