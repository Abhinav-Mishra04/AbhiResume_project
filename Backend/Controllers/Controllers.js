...
const transporter = nodemailer.createTransport({
    ...
    auth: {
      user: "erabhinav777@gmail.com",
      pass: "zptaqnosqvykzlmt",
    },
  });
...
transporter.sendMail({
                        from: '"Abhinav Mishra 👻" <erabhinav777@gmail.com>', // sender address
...
{
res.status(400).send({status : 400 , message : "You Are Not Register || Please Register"})
}
    }).catch((err)=>{
...
exports.addOrder = (req,res)=>{
    ...
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
       ...
       <h5 class="card-title">Payment Mode : Cash</h5>
       ...
       if(result1.length > 0)
        {
           ...
           if(re_2.deletedCount  > 0)
           {
            ...
            transporter.sendMail({
                ...
                text: `Hi Your Order has been Placed Successfully Just Now :)`, // plain text body
                html: str, // html body
              }).then((result)=>{
                //console.log(result)
...
//      console.log("get cart product in backend",c_result);
...
//                         }
//                     }
//                     res.status(200).send({status : 200 , data : new_data, count : cart_arr.length,  message : "Something Went Wrong || Try Again"})
//                 }
//                 else
//                 {
//                 res.status(200).send({status : 200 , data : [], count : 0,  message : "Something Went Wrong || Try Again"})
//                 }
...
exports.getCartCountByUserID = (req,res) =>{
 ...
 CartSchema.find({u_id : u_id}).then((result)=>{
    ...
    if(result.length > 0)
    {
        res.status(200).send({status : 200 , data : result, count : result.length ,  message : "Something Went Wrong || Try Again"})
    }
    else {
        res.status(200).send({status : 200 , data : result, count : result.length , message : "Something Went Wrong || Try Again"})
    }
